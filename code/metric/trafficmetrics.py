...
class DelayMetric(TrafficMetric):
    def __init__(self, _id, incoming_lanes, mode, lane_lengths, lane_speeds):
        ...
        self.lane_speed = {lane:float(lane_speeds[lane]) for lane in incoming_lanes}
        self.old_v = set()
        self.v_info = {}
        self.t = 0
    ...
    def get_v_delay(self, v):
        ...
        '''
        ## last step
        delay_last_step = max((self.v_info[v]['lane_speed'] - self.v_info[v]['last_speed']), 0)
        delay_cur_step = max((self.v_info[v]['lane_speed'] - self.v_info[v]['cur_speed']), 0)
        return (delay_last_step+delay_cur_step)/2
    ...
    def update(self, v_data):
        '''  update the veh info to calculate the metric
        '''
        new_v = set()

        ## update veh infos
        for lane in self.incoming_lanes:
            for v, v_info in v_data[lane].items():
                ## for new vehs
                if v not in self.old_v:
                    self.v_info[v] = {}
                    # self.v_info[v]['t'] = self.t
                    self.v_info[v]['lane'] = lane
                    ## lane speed: speed limits of the lane
                    self.v_info[v]['lane_speed'] = self.lane_speed[lane]
                    ## for new vehs: last_speed = speed limits
                    self.v_info[v]['last_speed'] = self.lane_speed[lane]
                    ## cur_speed 
                    self.v_info[v]['cur_speed'] = v_info[traci.constants.VAR_SPEED]
                ## for old vehs
                if v in self.old_v:
                    ## for old vehs: update last_speed
                    self.v_info[v]['last_speed'] = self.v_info[v]['cur_speed']
                    ## cur_speed 
                    self.v_info[v]['cur_speed'] = v_info[traci.constants.VAR_SPEED]
            
            ## update new_v for each lane
...
class QueueMetric(TrafficMetric):
    ...
    def get_metric(self):
        ...
        queue = sum([self.lane_queues[lane] for lane in self.lane_queues])
        ## append
